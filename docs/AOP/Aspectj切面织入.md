使用AspectJ将自定义切面来织入已有jar

**命令行方式**

```shell
java -jar $ASPECTJ_TOOLS -cp $MY_ASPECT:$ASPECTJ_RT -source 1.8 -inpath target/classes -d target/classes -aspectpath $MY_ASPECT
或者 
java -jar $ASPECTJ_TOOLS -cp $MY_ASPECT:$ASPECTJ_RT -source 1.8 -inpath  $IN_JAR -outjar $OUT_JAR -aspectpath $MY_ASPECT
#其中 $ASPECTJ_TOOLS 为aspectjtools-1.8.9.jar所在位置
#$ASPECTJ_RT为aspectjrt-1.8.9.jar所在位置
#$MY_ASPECT为要织入的切面所在的jar包
...
<plugin>
    ...
    <configuration>
        ...
        <!-- 在aspectLibraries子标签指定自定义切面jar包 -->
        <aspectLibraries>
            <aspectLibrary>
                <groupId>com.ginkoc.learning</groupId>
